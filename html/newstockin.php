            ...
            <div class="card-body">
              ...
              <div class="app-brand justify-content-center">
                <a href="#" class="app-brand-link gap-2">
                  ...
                  <span class="app-brand-text demo text-capitalize text-body fw-bolder"><?php echo $lang["addstock"]; ?></span>
                </a>
              </div>
              <!-- /Logo -->
              
              <form id="formAuthentication" class="mb-3" action="newstockin.php" method="POST">
                <div class="mb-3">
                  ...
                  <select class="form-select"  name="pname" required>
                    ...
                    $prod=mysqli_query($con,"SELECT * FROM products");
                      ...
                      while ($row=mysqli_fetch_array($prod)) {
                        ?>
                        <option class="text-capitalize" value="<?php echo $row["p_id"]; ?>"><?php echo $row["product_name"].$lang["stquality"].$row["quality"].$lang["stmanufacturer"].$row["manufacturer"]; ?></option>
                        <?php
                    }
                    ?>
                  </select>
...
<?php
    if (isset($_POST["save"])) {
      include "../includes/connect.php";
      $date=date("Y-m-d");
             $insert=mysqli_query($con,"INSERT INTO stock_in VALUES('','$_POST[pname]','$_POST[quantity]','$_POST[unit_price]','$_POST[sell_price]','$date','$_POST[expiration]')");
        if ($insert) {
          
          echo "<script>alert('".$lang["stockinsucess"]."')</script>";
        }
      }
    ?>
            <!-- /php -->
